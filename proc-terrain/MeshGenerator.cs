...
namespace ProcWorld
{
    public class MeshGenerator
    {
        ...
        public static MeshData GenerateMeshFromHeightMap(HeightMap _heightmap, float _heightScale, AnimationCurve _heightCurve, int _lod)
        {
            ...
            for (int y = 0; y < height; y += currentIncrement)
            {
                for (int x = 0; x < width; x += currentIncrement)
                {
            ...
            for (int y = 0; y < height; y += increment)
            {
                for (int x = 0; x < width; x += increment)
                {
                    ...
                    float xPos = topLeftX + normalizedX * meshTargetPhysicalSize;
                    float zPos = topLeftZ - normalizedZ * meshTargetPhysicalSize;
...
// Not a fan of how this works but the results are quite acceptable. 
// Explanation: LODs are generated by skipping vertices, but since we have border vertices to calculate seamless normals it means we end up
// skipping the physical borders of the mesh
// to prevent the gaps, we expand the mesh 
// but this means the noise sampling is now out of sync with the physical position of the mesh
// to fix that, we use the physical xPos and zPos to get the sampling coordinates for the noise 
// but importantly, we only do this for physical vertices, skipping borders becasue after the mesh is expanded
// the physical position of the invisible border vertices goes out of the range of the noise map. 

// this was a consequence of the fact that the initial LOD system didn't account for seamless normals between chunks 
// preferably, the LOD system will be reworked to simplify normal calculation without having to create phantom triangles
// as far as physical/shape accuracy goes, it's as perfect as it can be since the positions are used to sample the height 
// but, uvs don't account for this and probably neither the normals. Which should be fine in practice, especially if you're using shaders that don't rely on uvs
// such as triplanar  
                    if (!isBorder)
                    {
                        sx = Mathf.RoundToInt(xPos + (width - 1) / 2f);
                        sy = Mathf.RoundToInt((height - 1) / 2f - zPos);
                    }
